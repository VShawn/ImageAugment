...
class SingleImageAugmenter(object):
    ...
    新建实例时先配置除了 Step1_RotateCount 外的参数，再 SetAugmentCount 设置扩充目标数量，自动计算旋转次数
    从 json 读取时，扩充目标数量已设置好了，不需要单独调用 SetAugmentCount 。
    '''
    def __init__(self) -> None:
        ...
        self.SetAugmentCount(5000) # 设置扩充目标数量，并根据扩充目标数量计算旋转次数
